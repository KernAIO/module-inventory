...
Rebuild the asset register on a schema that applies and a list that really pages
...
`pnpm db:generate` away rather than already in the tree; this change *is* that regeneration, and
...
cannot apply *standalone* — `0001_rls.sql` reaches for `btree_gist` and nothing had created it, so
...
which removes the crash and *not* the problem. A replayed `create table if not exists` reports
...
`assets.list` pages for real. It takes an opaque cursor and orders by the sort column *and* the id —
...
*string*, and the id is interpolated into a `::uuid` cast, so a marker holding `not-a-uuid` reached
...
`assets.update` is a patch, and every editable field reads the same way now: `undefined` means *not
mentioned* and keeps what is there, `null` means *clear it*. `description` did not, so a patch that
