...
- 4c3e235: Scope the demo seeder's emptiness guard to the workspace.
  ...
  policy it does not, so the guard saw the _previous_ workspace's rows and reported an empty workspace
...
- 5bda66a: Fill a workspace created with example content: forty items across six categories, three workspace
...
- dff8780: Bind the `'*'` sentinel for the registry read the nightly sweeps start from
...
- test(inventory): bless the permission matrix

## 0.5.0

### Minor Changes

- f33881b: Finish the register: custom fields, lost and retired items, and the last unreachable corners.
